...
\paragraph{Dissonanz / Inharmonie}
...
\end{figure}
In Essentia gibt es explizit einen Algorithmus, der aus den Spitzen des Spektrums dessen Dissonanz berechnet. Dazu wird die paarweise Dissonanz zweier spektraler Spitzen errechnet und deren Durchschnitt gebildet. \cite{EssentiaDissonance}

\newpage

\subsection{Music Emotion Recognition}
Music Features to use, Stand der Technik, Arousal Recognition, Valence Recognition

Der Versuch erzeugte Emotionen aus der Musik zu gewinnen wird ``Music Emotion Recognition'' genannt. Dabei werden nach den modernsten Methoden verschiedene Lowlevel und Highleven Features aus der Musik extrahiert um diese dann mit Machine Learning Algorithmen auszuwerten. Entweder probiert man hierbei die Musik einer Klasse zuzuordnen, wie beispielsweise ``Friedlich'' oder ``Hektisch, Aggressiv'' oder es wird versucht die Features der Musik auf ein Model von menschlichen Emotionen abzubilden.
